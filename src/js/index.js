...
APP.init = () => {
    ...
    if(window.location.href.includes('#debug')) {
        ...
        APP.canvas.u_glowintensity = { value: .001 }
        ...
        APP.__root.querySelector('#u_u').insertAdjacentHTML('beforeend', '<div class="u"><input class="u_glowintensity uu" placeholder="glow int" type="number"></div>')
        ...
        APP.__root.querySelector('input.u_glowintensity').addEventListener('keypress', e => (e.key === 'Enter' && e.keyCode === 13) && new Animation({ onUpdate: function() {
            APP.canvas.shader.setUniform('u_glowintensity',  APP.canvas.u_glowintensity.value);
        } }).to(APP.canvas.u_glowintensity, 5, {
            value: e.target.value
        }))
    }
    ...
    new Animation({ onUpdate: function() {
        ...
        value: APP.canvas.sketch.width / 1.5 < 370 ? APP.canvas.sketch.width / 1.5 : 400
    })
